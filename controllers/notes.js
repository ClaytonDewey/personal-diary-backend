...
const updateNote = (req, res) => {
  ...
  Note.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then((data) => {
      if (!data) {
        res.status(404).send({
          message: `Cannot update note with id=${id}. Not was not found!`,
        });
      } else res.send({ message: "Note was updated successfully!" });
    })
    .catch((err) => {
      res.status(500).send({
        message: `Error occured while updating note with id=${id}`,
      });
    });
};
...
const getNote = (req, res) => {
  ...
  Note.findById(id)
    ...
    .catch((err) => {
      res.status(500).send({ message: `Error fetching note with id=${id}` });
    });
};
